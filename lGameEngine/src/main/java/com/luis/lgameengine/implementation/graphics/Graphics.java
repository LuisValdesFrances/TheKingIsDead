...
public class Graphics {
    ...
    public void drawRegion(Image image,
        ...
        int extraPosFlipH = flipH ? getExtraFlipH(destX, srcWidth, anchor) : 0;
        int extraPosFlipV = flipV ? getExtraFlipV(destY, srcHeight, anchor) : 0;

        screenRect.left = srcX;
        screenRect.top = srcY;
        screenRect.right = srcX + srcWidth;
        screenRect.bottom = srcY + srcHeight;

        destinyRect.left = destX - this.getExtraX(srcWidth, anchor)-extraPosFlipH;
        destinyRect.top = destY - this.getExtraY(srcHeight, anchor)-extraPosFlipV;
        destinyRect.right = destX + destWidth - this.getExtraX(srcWidth, anchor)-extraPosFlipH;
        destinyRect.bottom = destY + destHeight - this.getExtraY(srcHeight, anchor)-extraPosFlipV;
    ...
    private int getExtraFlipH(int x, int imageWidth, int anchor) {
        int extra = 0;
        if ((anchor & LEFT) != 0) {
            extra = imageWidth;
        }
        ...
        return ((x - canvas.getWidth() / 2) * 2) + extra;
    }

    private int getExtraFlipV(int y, int imageHeight, int anchor) {
        int extra = 0;
        if ((anchor & TOP) != 0) {
            extra = imageHeight;
        }
        ...
        return ((y - canvas.getHeight() / 2) * 2) + extra;
    }

    private int getExtraY(int imageHeight, int anchor) {
        int extra = 0;
        if ((anchor & BOTTOM) != 0) {
            extra = imageHeight;
        }
    ...
    private int getExtraX(int imageWidth, int anchor) {
        int extra = 0;
        if ((anchor & RIGHT) != 0) {
            extra = imageWidth;
        }
